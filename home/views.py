...
def new_lat_lon(coordinates):
    ...
    # final_sens = sen_locc.objects.all()
    # for sens in final_sens:
    #     lat_str = "{:.5f}".format(sens.lat)  # Convert latitude to string with 4 decimal places
    #     lon_str = "{:.5f}".format(sens.lon)  # Convert longitude to string with 4 decimal places
    #     lat_truncated = float(lat_str[:lat_str.index('.') + 6])  # Keep only the first 4 digits after the decimal point
    #     lon_truncated = float(lon_str[:lon_str.index('.') + 6])  # Keep only the first 4 digits after the decimal point
    #     sensor_coor.append((lat_truncated, lon_truncated))
    
    # print("all sensor:", sensor_coor)
